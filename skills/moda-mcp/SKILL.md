...
7. **Design tasks auto-export — don't re-export.** A completed `start_design_task` / `remix_design` returns the finished design already rendered to a file at `result.export` (`{url, format, status, page_count}`). Read that; only call `export_canvas` for a *different* format or page.
8. **`export_canvas` `{status: "not_ready"}` is a retry state, not an error** — returned while a design task is still running on the canvas. Retry after `retry_after_seconds`.
9. **Large exports return `{status: "in_progress", task_id}`** when they exceed the sync budget. Poll `get_export_status` for the URL.
10. **Brand-kit resolution order** on `start_design_task` / `remix_design`: explicit `brand_kit_id` → session preference (from `set_session_brand_kit` or the showcase iframe's "Use for this session" button) → team default → none (only if `skip_brand_kit=true`). Don't restate colors / fonts / logos in the prompt — the resolved kit owns them.
11. **`find_brand_kits` for lookups; `list_brand_kits` only when the user asks to see them.** Both return the same data, but `list_brand_kits` renders a visual showcase iframe on **every** call (per the MCP Apps spec, can't be suppressed per-call). Use `find_brand_kits` when picking a `brand_kit_id` for another tool.
12. **Attachments have two shapes**: `{file_id, role}` (preferred — carries `role` ∈ `source` / `reference` / `asset` that changes agent behavior) and `{url, type}` (legacy, drops role metadata).
